...
import { FieldIconName, ColorTheme } from '../../types';

interface AddCustomFieldModalProps {
  isOpen: boolean;
  onClose: () => void;
  onAdd: (name: string, description: string, iconName: FieldIconName, colorTheme: ColorTheme) => void;
...
export const AddCustomFieldModal: React.FC<AddCustomFieldModalProps> = ({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    onAdd(
      trimmedName,
      description.trim() || 'Custom academic field track',
      iconName,
      colorTheme
    );
    setName('');
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <input
                ...
                placeholder="e.g. Computer Science, Humanities, Commerce, Law"
                autoFocus
                maxLength={40}
            ...
            <div>
              <label className="block text-xs font-semibold uppercase tracking-wider text-slate-600 dark:text-slate-300 mb-1.5">
                Description / Core Subjects (Optional)
              </label>
              <input
                type="text"
                value={description}
                onChange={(e) => setDescription(e.target.value)}
                placeholder="e.g. Programming, Mathematics, English"
                maxLength={80}
                className="w-full px-3.5 py-2.5 rounded-xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-white text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500"
              />
            </div>
